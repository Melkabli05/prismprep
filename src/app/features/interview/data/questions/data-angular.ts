      ...
      questions: [
        {
          ...
          deepDive: "Les route guards contrôlent quelles routes un utilisateur peut accéder.\n## Les 4 types\n**CanActivate** : autorise ou bloque l'accès à une route\n**CanDeactivate** : confirmation avant de quitter (formulaire modifié)\n**CanLoad** : empêche le chargement lazy du chunk JS\n**Resolve** : pré-charge les données avant l'affichage\n## CanActivate\n```typescript\n@Injectable({ providedIn: 'root' })\nexport class AuthGuard implements CanActivate {\n  canActivate(): boolean {\n    if (this.auth.isLoggedIn()) return true;\n    this.router.navigate(['/login']);\n    return false;\n  }\n}\n```\n## Resolve\n```typescript\nexport class ArticleResolver implements Resolve<Article> {\n  resolve(route: ActivatedRouteSnapshot): Observable<Article> {\n    return this.api.getArticle(route.param['id']);\n  }\n}\n// Dans la route : resolve: { article: ArticleResolver }\n```\n## Sécurité — garde cliente uniquement\nLes guards tournent côté navigateur et peuvent être contournées par un utilisateur déterminé. Toujours combiner avec une vérification serveur des autorisations.",
        },
        {
          id: 'ng-5',
