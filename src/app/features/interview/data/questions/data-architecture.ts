...
- **Controllercouplage** : controller qui instancie directement des dépendances plutôt que de les recevoir.
...
5. **Validation en entrée** : valider les paramètrès dès le Controller.
...
6. **Sous-estimer l'effort de collaboration** : DDD implique des ateliers réguliers avec les métiers, ce qui à un coût.
...
**Avantages :**
- **Maintenabilité** : chaque classe à une raison unique de changer, les impacts sont localisés.
...
5. **DIP vià un container IoC** : laisser un framework (Spring, Guice) gérer l'injection.
...
**Il ne doit PAS** appeler les méthodes d'objets retournés par d'autrès objets (les « amis des amis »).
...
CAP ne dit pas que la cohérence où la disponibilité est binaire. On peut avoir :
